...
func (r *CenterRepo) Get(ctx context.Context) (*domain.Center, error) {
	...
	err := r.db.QueryRowContext(ctx,
		...
		&c.SlotDuration, &c.MaxPatientsDay, &c.SetupComplete, &created, &updated)
...
func (r *CenterRepo) CompleteSetup(ctx context.Context) error {
	_, err := r.db.ExecContext(ctx, `UPDATE center SET setup_complete = 1, updated_at = datetime('now') WHERE id = 1`)
	return err
}
